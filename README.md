...
默认依赖已包含本地 BGE rerank。Intel Mac 会固定使用可安装的 `torch==2.2.2`。
...
默认开启 BGE rerank 二阶段重排。`SIMILARITY_TOP_K` 控制第一阶段从 Qdrant 召回的候选 chunk 数量，`RERANK_TOP_N` 控制 rerank 后保留给大模型的 chunk 数量。如果本机没有 GPU，保持 `RERANK_USE_FP16=false`；使用 CUDA GPU 时可以改为 `true`。

首次执行 `ask` 时会加载 `RERANK_MODEL` 对应的本地 reranker 模型；如果模型尚未缓存，会先从 Hugging Face 下载。内网环境可以提前下载模型，并把 `RERANK_MODEL` 配成本地模型目录。
